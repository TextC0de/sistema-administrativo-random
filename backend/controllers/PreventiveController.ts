...
const PreventiveController = {
	putPreventive: async (req: NextConnectApiRequest, res: NextApiResponse<ResponseData>) => {
		const { body } = req
		await dbConnect()
		const { _id, branch, business, assigned, status, frequency, months, lastDoneAt, batteryChangedAt, observations } =
			body
		...
		try {
			...
			res.json({
				statusCode: 200,
				data: { Preventive: formatIds(newPreventive), message: 'updated Preventive succesfully' }
			})
		} catch (error) {
			console.log(error)
	...
	postPreventive: async (req: NextConnectApiRequest, res: NextApiResponse<ResponseData>) => {
		...
		try {
			...
			if (deletedPreventive != null) {
				...
				return res.json({
					statusCode: 200,
					data: { preventive: formatIds(deletedPreventive), message: 'created Preventive succesfully' }
				})
			}
			...
			return res.json({
				statusCode: 200,
				data: { preventive: formatIds(newPreventive), message: 'created Preventive succesfully' }
			})
		} catch (error) {
			console.log(error)
