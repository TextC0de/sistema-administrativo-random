...
const CityController = {
	putCity: async (req: NextConnectApiRequest, res: NextApiResponse<ResponseData>) => {
		...
		if (newCity == null) return res.json({ statusCode: 500, error: 'could not update city' })
	...
	deleteCity: async (req: NextConnectApiRequest, res: NextApiResponse<ResponseData>) => {
		...
		if (deletedCity == null) return res.json({ statusCode: 500, error: 'could not delete city' })
