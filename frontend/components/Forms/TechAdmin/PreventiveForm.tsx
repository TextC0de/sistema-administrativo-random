...
const PreventiveForm = ({ preventiveForm, newPreventive = true, businesses, branches, clients, technicians }: props): JSX.Element => {
    const router = useRouter()
    const [errors, setErrors] = useState<IPreventiveFormErrors>({} as IPreventiveFormErrors)
    const [submitted, setSubmitted] = useState<boolean>(false)
    const [client, setClient] = useState(preventiveForm.branch.client !== undefined ? preventiveForm.branch.client.name : '')
 ...
 */
    ...
    const addMonth = (e: ChangeEvent<HTMLSelectElement>): void => {
        const { value } = e.target
        console.log(value)
        setForm(prev => { return { ...prev, months: !prev.months.some(month => month === value as types.Month) ? [...prev.months, value as types.Month] : prev.months } })
    }
            ...
            <form id='task' className='flex flex-col my-4 mx-auto w-1/2 bg-gray-50 rounded-3xl p-4' onSubmit={handleSubmit}>
                ...
                <div id='select-branch'>
                    ...
                    <Select
                        ...
                        color={errors.branch !== '' ? 'failure' : ''}
                    >
                        <option value="default" hidden disabled>{newPreventive ? 'Seleccione una sucursal...' : `${preventiveForm.branch.number.toString()}, ${preventiveForm.branch.city.name}`}</option>
                ...
                <div id='select-frequency'>
                    ...
                    >
                        <option value="default" hidden disabled>{newPreventive ? 'Seleccione la frequencia' : preventiveForm.frequency !== undefined ? preventiveForm.frequency > 1 ? `Cada ${preventiveForm.frequency} meses` : 'Todos los meses' : 'Seleccione la frequencia'}</option>
                            ...
                            return (
                                <li className='rounded-full bg-gray-300 py-2 px-3 mr-1 mb-2 inline-block' key={index}>
                                    <div className='flex justify-between items-center gap-2 font-semibold'>
                                        {technician.fullName}
                                        <button className='rounded-full bg-white ' type='button' onClick={() => deleteTechnician(technician._id as string)}>
