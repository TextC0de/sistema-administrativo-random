			...
			<form className="flex flex-col gap-4 bg-gray-50 rounded-3xl p-4 mx-auto w-1/2 mt-4" onSubmit={handleSubmit}>
				...
				<div>
					<div className="mb-2 block">
						<Label htmlFor="firstName" value="Nombre del usuario" className="text-lg" />
					</div>
					<TextInput
						id="firstName"
						name="firstName"
				...
				<div>
					<div className="mb-2 block">
						<Label htmlFor="lastName" value="Apellido del usuario" className="text-lg" />
					</div>
					<TextInput
						id="lastName"
						name="lastName"
				...
				<div>
					<div className="mb-2 block">
						<Label htmlFor="email" value="Email del usuario" className="text-lg" />
					</div>
					<TextInput
						id="email"
						name="email"
				...
				<div className="flex flex-col gap-4" id="checkbox">
					...
					<div className="mb-2 block">
						<Label htmlFor="roles error" value={errors.roles} className="text-lg" color="failure" />
					</div>
				</div>
				<div id="select-city">
					<div className="mb-2 block">
						<Label htmlFor="city" value="En que ciudad reside el usuario?" className="text-lg" />
					</div>
					<Select id="cities" onChange={selectCity} name="city" defaultValue="default">
						<option value="default" disabled hidden>
							{newUser
								? 'Seleccione una ciudad'
								: userForm.city !== undefined && `${userForm.city.name}, ${(userForm.city.province as IProvince).name}`}
						</option>
						{cities.map((city, index) => (
							<option key={index}>{`${city.name}, ${(city.province as IProvince).name}`}</option>
						))}
					</Select>
				</div>
