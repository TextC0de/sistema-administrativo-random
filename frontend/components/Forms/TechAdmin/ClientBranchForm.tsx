...
export default function ClientBranchForm({ branchForm, newBranch = true, cities, businesses }: props): JSX.Element {
	...
	const deleteBranchBusiness = (id: string): void => {
		setForm((prev) => {
			return { ...prev, businesses: prev.businesses.filter((business) => business._id !== id) }
		})
	}
			...
			>
				<h2 className="text-lg">
					{`${branchForm.client.name} : ${
						newBranch ? 'Agregar una sucursal' : `Editar la sucursal  ${branchForm.number}`
					}`}
				</h2>
				<div>
					<div className="mb-2 block">
						<Label htmlFor="number" value="Numero de la sucursal" className="text-lg" />
					</div>
					<TextInput
						id="number"
						type="number"
				...
				<div id="select-city">
					<div className="mb-2 block">
						<Label htmlFor="city" value="Elegi la ciudad" className="text-lg" />
					</div>
					<Select
						id="citys"
						required={true}
					...
					>
						<option value="default" disabled hidden>
							{newBranch
								...
								: `${form.city.name}, ${(form.city.province as IProvince).name}`}
						</option>
				...
				<div>
					...
					<div className="mb-2 block">
						<Label htmlFor="business error" value={errors.businesses} className="text-lg" color="failure" />
					</div>
				</div>
