...
export default function PreventiveTable({
	...
	function selectEntity(e: ChangeEvent<HTMLSelectElement>): void {
		...
		switch (type) {
			...
			case 'Provincia':
				setPreventivesTable(
					preventives.filter((preventive) => (preventive.branch.city.province as IProvince).name === value)
				)
				break
			case 'Tecnico':
				setPreventivesTable(
					preventives.filter((preventive) => preventive.assigned.some((tech) => tech.fullName === value))
				)
				break
			case 'Empresa':
