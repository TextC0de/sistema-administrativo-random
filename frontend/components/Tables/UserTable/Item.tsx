...
export default function Item({ user, deleteUser }: props): JSX.Element {
	...
	async function reGeneratePassword(): Promise<void> {
		try {
			...
			triggerAlert({
				type: 'Success',
				message: `Se generó una nueva contraseña para ${user.fullName as string} correctamente`
			})
			stopLoading()
		} catch (error) {
			console.log(error)
			...
			triggerAlert({
				type: 'Failure',
				message: `No se pudo generar una nueva contraseña para ${user.fullName as string}`
			})
		}
	}
	...
	return (
		<Table.Row className="border-b ">
			...
			<Table.Cell>
				{user.city != null ? `${user.city?.name}, ${(user.city?.province as IProvince).name}` : ''}
			</Table.Cell>
			<Table.Cell>{user.email}</Table.Cell>
			<Table.Cell>
				{(user.roles?.length as number) > 1 ? user.roles?.map((role) => `${role}, `) : user.roles?.[0]}
			</Table.Cell>
			<Table.Cell>
